...
```{r warning=FALSE}
reg_logistica <- glm(formula=situacao~.,
                     data = smote_train, 
                     family=binomial)
...
TP <- test_data %>% filter(situacao == "eleito", prediction == "eleito") %>% nrow()

# vezes que o modelo acertou na predição de que o candidato seria nao_eleito
TN <- test_data %>% filter(situacao == "nao_eleito" , prediction == "nao_eleito" ) %>% nrow()

# vezes que o modelo errou na predição de que o candidato seria nao_eleito
FP <- test_data %>% filter(situacao == "nao_eleito" , prediction == "eleito") %>% nrow() 

# vezes que o modelo errou na predição de que o candidato seria eleito
FN <- test_data %>% filter(situacao == "eleito", prediction == "nao_eleito" ) %>% nrow()
...
precision
recall
confusionMatrix(test_data$prediction, test_data$situacao)
varImp(adaboost)
```
